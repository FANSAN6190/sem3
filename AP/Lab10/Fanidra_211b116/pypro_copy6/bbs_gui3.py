...
class journey:
    def __init__(self) :
        ...
        def home():
            ...
            hm=home()
        Button(mid_frame,text="Show Bus",command=lambda:self.sel_bus(root),font="Ariel 12 bold ",bg='SpringGreen3',activebackground="SpringGreen3",bd=5).grid(row=3,column=10,padx=10)
        Button(mid_frame,image=self.img2,bd=3,command=home).grid(row=3,column=11,padx=10)
        root.mainloop()
    
    #-------------Select Bus---------------
    def sel_bus(self,root):
        ...
        cur.execute('select DISTINCT b.bid,o.oname,b.bus_type,b.capacity,b.fare, r1.staid,r1.sta_name,r2.staid,r2.sta_name from route as r1,route as r2,run as rn,buses as b,operator as o where r1.sta_name="{0}" and r2.sta_name="{1}" and run_date="{2}" and r1.staid>r2.staid and b.rid=r1.rid and b.bid=rn.bid and b.oid=o.oid;'.format(self.to_st,self.source,self.tr_on))
        ...
        Button(select_bus,text="Proceed to Book",command=lambda:self.book(root),font="Ariel 12 bold ",bg='lime green',activebackground="lime green",bd=3).grid(row=6,column=10,padx=60)
    ...
    def book(self,root):
        ...
        Button(booking,text="Book Seat",font="Ariel 12 bold ",bg='light green',activebackground="light green",command=lambda:self.rec(self.bus_select),bd=3).grid(row=2,column=14,padx=10)
    def rec(self,x):
        n=self.name.get()
        g=self.gen.get()
        s=self.seats.get()
        ...
        print(bref)
        query='insert into pass_booking(book_ref,pname,gender,age,phone,seats,travel_on,booked_on,boarding_point,bus_operator,fare) values({0},"{1}","{2}",{3},"{4}",{5},"{6}",DATE(),"{7}","{8}",{9})'.format(bref,n,g,a,m,s,self.tr_on,self.source,op,fr)
        print(query)
        o=self.qx(query)
        if o==True and x==1:
